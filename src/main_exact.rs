...
fn read_sequence_from_file(path: &str) -> std::io::Result<Vec<u8>> {
        ...
        match b {
            ...
            _ => {
                return Err(std::io::Error::new(
                    ...
                    format!("Invalid base '{}' in {}", b as char, path),
                ))
            }
        }
    }
    if sequence.is_empty() {
        return Err(std::io::Error::new(
            ErrorKind::InvalidData,
            format!("No DNA bases found in {}", path),
        ));
    }
    Ok(sequence)
}

fn find_random_valid_window(sequence: &[u8], window_len: usize) -> Option<usize> {
    if window_len > sequence.len() {
        return None;
    }
    let is_valid = |b: u8| matches!(b, b'A' | b'C' | b'G' | b'T');
    let mut invalid = 0usize;
    let mut rng = rand::thread_rng();
    let mut chosen: Option<usize> = None;
    let mut seen = 0usize;
    for &b in &sequence[..window_len] {
        if !is_valid(b) {
            invalid += 1;
        }
    }
    ...
    for i in window_len..sequence.len() {
        if !is_valid(sequence[i - window_len]) {
            invalid -= 1;
        }
...
fn main() {
    let matches = App::new("basic_seed_chainer")
        ...
        .arg(
            Arg::with_name("sequence_file")
                ...
                .takes_value(true)
                .required(true)
                .help("Path to a plain DNA sequence file to use for S"),
        )
        .get_matches();
    ...
    let threads = matches
        ...
        .parse::<usize>()
        .unwrap();
    let sequence_file = matches.value_of("sequence_file").unwrap();
    let full_sequence =
        read_sequence_from_file(sequence_file).expect("Failed to read sequence file");
    ...
    for theta in thetas {
        ...
        let theta_capped = if theta > 0.159 { 0.159 } else { theta };
        let alpha = -((1.0 - theta_capped) as f64).log(4.0);
        let _C = 3. / (1. - 2. * alpha);
        let ks = 20..max_k; 
        ...
        for k in ks {
            ...
            let n = n as usize;
            let gamma = 0.50;
            if n > full_sequence.len() {
                panic!(
                    "Requested n = {} exceeds sequence length {} from {}",
                    n,
                    full_sequence.len(),
                    sequence_file
                );
            }
            let s_start = find_random_valid_window(&full_sequence, n).unwrap_or_else(|| {
                panic!(
                    "No window of length {} with only A/C/G/T in {}",
                    n, sequence_file
                )
            });
            let s = full_sequence[s_start..s_start + n].to_vec();
            ...
            if m_is_substring {
                ...
                m = (1. * (n as f64).powf(avg)) as usize;
            } else {
                start_ind = 0;
                m = n as usize;
            }
            ...
            (0..num_iters)
                ...
                .for_each(|_l| {
                    ...
                    let recoverability =
                        simulation_utils::get_exact_recoverability(&best_chain, &hom_path, k);
